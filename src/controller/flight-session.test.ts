/**
 * Guards `FlightSession` initialization, input ownership, and live state changes.
 * These cases exercise failure rollback, pause/camera presentation, and cleanup
 * using a hosted-scene double without creating a real ArcGIS view.
 */
...
function createHostedScene(): {
  ...
  // Keep the session test focused on lifecycle behavior with browser-like event targets and no real ArcGIS view.
...
describe("FlightSession lifecycle", () => {
  it.each([undefined, 200_000])("applies the %s AGL ceiling relative to sampled ground", maximumAglM => {
    ...
    // The configured ceiling is above ground level, so the expected world Z includes the sampled terrain elevation.
