/**
 * Guards the deterministic classic-aircraft flight step.
 * These cases replay control sequences and check the speed envelope, turbo
 * and brake effects, commanded attitude, and self-leveling after release.
 */
...
describe("deterministic flight behavior", () => {
  it("advances an identical input sequence deterministically", () => {
    // Running the same fixed-step replay twice catches hidden dependence on wall-clock time or mutable shared state.
    const run = () => advance(
  ...
  it("preserves the normal speed envelope and turbo/brake behavior", () => {
    ...
    // Braking must interrupt the temporary turbo boost immediately, rather than only limiting later acceleration.
