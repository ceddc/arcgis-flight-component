/**
 * Render the optional flight toolbar inside an isolated shadow root. The owning
 * custom element supplies actions and session snapshots; this module translates
 * those snapshots into localized button labels, speed text, and accessible
 * selected/disabled states. It owns only its DOM and input listeners.
 */
import type {
...
/** Commands delegated to the owning custom element when toolbar controls activate. */
export interface FlightControlsActions {
  /** Change the aircraft's slow/normal/turbo power mode. */
  setPowerMode(mode: FlightPowerMode): void;
  /** Pause, resume or start the current session according to its phase. */
  togglePause(): void;
  /** Switch between chase and cockpit camera modes. */
  setCameraMode(mode: FlightViewMode): void;
  /** Recover the aircraft to the session's safe flight state. */
  recover(): void;
}

/** Ordered radio choices; shared with the toolbar's arrow/Home/End navigation. */
...
/** Check whether a configured toolbar control should be rendered. */
...
/**
 * Render and update the accessible flight toolbar inside an isolated shadow root.
 *
 * The overlay rebuilds only when its structure or locale changes. Session
 * snapshots update labels, speed and enabled/selected states without replacing
 * the toolbar DOM on every simulation frame.
 */
