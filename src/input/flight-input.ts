/**
 * Collects scene-scoped keyboard, gamepad, touch, and host-provided flight controls.
 *
 * The controller captures flight keys only while its scene owns interaction,
 * leaves editable/activation controls to the host UI, and chooses one active
 * standard gamepad. `sample()` turns these sources into one bounded control
 * frame; host patches override individual fields until cleared.
 */
...
/** Attack/release smoothing and relative yaw strength for digital keyboard input. */
...
/** Normalized input sampled by the session: axes are `[-1, 1]`, actions are `[0, 1]`. */
...
/** Optional host values that override only their matching sampled control fields. */
export type FlightControlPatch = Partial<FlightControlFrame>;

/** Runtime input switches and sensitivity values after defaults are applied. */
...
/** Immutable input defaults shared by new controllers. */
...
/** Minimal standard-mapped gamepad snapshot used by the input selector. */
...
/** Selected gamepad identity exposed in session snapshots. */
...
/** DOM target, live settings, optional testable gamepad provider, and action callbacks. */
...
/** Classification used to leave text editing and native activation keys to host UI. */
...
/** Applies defaults and clamps sensitivity/dead-zone settings to supported bounds. */
...
/** Removes center drift and rescales remaining stick travel to the full `[-1, 1]` range. */
export function applyGamepadDeadzone(
  value: number,
  threshold = DEFAULT_GAMEPAD_DEADZONE,
): number {
  // Rescale the surviving range so full stick deflection still maps to full control.
...
/** Scores a standard gamepad's usable stick/button activity for multi-pad selection. */
export function flightGamepadActivity(
  ...
  const button = gamepad.buttons.reduce(
    ...
    (maximum, candidate, index) => gamepad.mapping === "standard" && index === 16
      ? maximum : Math.max(maximum, finiteButtonValue(candidate)),
    0,
  );
  return Math.max(primaryStick, button);
}

/** Keeps the current pad, accepts a sole standard pad, or chooses the most active ready pad. */
...
/** Identifies flight keys that may be prevented without stealing native control activation. */
...
/** Owns scene-scoped keyboard, gamepad, and touch input and emits normalized control frames. */
export class FlightInputController {
  ...
  setTouchStick(bank: number, pitch: number): void {
    this.touchBank = finiteAxis(bank);
    this.touchPitch = finiteAxis(pitch);
  }
  ...
  sample(deltaSeconds = 1 / 60): FlightControlFrame {
    ...
    const hardware: FlightControlFrame = {
      pitch: clamp(
        (this.keyboardPitch + (this.touchPitch - gamepadPitch) * settings.sensitivity) * pitchDirection,
        -1,
        1,
      ) || 0,
      bank: clamp(this.keyboardBank + (gamepadBank + this.touchBank) * settings.sensitivity, -1, 1),
      yaw: clamp(
        this.keyboardYaw
  ...
  private pollGamepad(
    ...
    // Require a near-center sample before an axis can steer or select among pads; a newly
    // connected controller can report a stale non-neutral position.
  ...
  private readonly handleKeyDown = (event: KeyboardEvent): void => {
    ...
    if (
      ...
      // A held flight key survives repeats redirected to another control in the scene.
      || (!(event.repeat && this.keys.has(event.code))
