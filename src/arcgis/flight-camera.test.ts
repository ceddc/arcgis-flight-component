/**
 * Exercise camera-frame generation from simulated aircraft poses.
 * Covers chase/cockpit framing and transitions, orbit input, terrain clearance
 * and viewport-dependent FOV so frames remain consistent across view modes.
 */
...
describe("flight camera controller", () => {
  ...
  it("moves the chase camera with pitch during climbs and dives", () => {
    // The chase offset follows the aircraft's pitch: a climb brings the camera closer, while a dive reveals more space below.
...
it("orbits chase, clears sampled ground, and resets the look on cockpit entry", () => {
  ...
  // Supply elevated sampled ground so the orbit path must preserve terrain clearance as well as the drag offset.
