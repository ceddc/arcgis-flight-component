/**
 * Exercise terrain and mesh acquisition during flight startup.
 * Checks elevation-unit conversion and cancellation of parallel glTF loads,
 * including disposal of meshes returned after an abort or sibling failure.
 */
...
describe("initialization resource cancellation", () => {
  it.each([
    ["meters", 1], ["feet", 0.3048], ["us-feet", 1200 / 3937],
  ] as const)("uses source vertical %s independently of horizontal feet", async (heightUnit, factor) => {
    const ground = groundWithQuery(vi.fn(async () => ({
      geometry: { z: 100 },
      sampleInfo: [{ demResolution: 1, source: {
        spatialReference: { isGeographic: false, metersPerUnit: 1200 / 3937 },
        heightModelInfo: { heightUnit },
      } }],
    })));
    await expect(queryInitialGroundElevation(ground, point)).resolves.toBeCloseTo(100 * factor);
  });

  it("uses projected source units when no vertical model is supplied", async () => {
    const ground = groundWithQuery(vi.fn(async () => ({
      geometry: { z: 100 },
      sampleInfo: [{ demResolution: 1, source: {
        spatialReference: { isGeographic: false, metersPerUnit: 1200 / 3937 },
    ...
    await expect(queryInitialGroundElevation(ground, point)).resolves.toBeCloseTo(100 * 1200 / 3937);
  });

  it("does not convert a no-data sentinel into terrain height", async () => {
    const ground = groundWithQuery(vi.fn(async () => ({ geometry: { z: -9999 }, noDataValue: -9999 })));
    await expect(queryInitialGroundElevation(ground, point)).resolves.toBeNull();
  });

  it("keeps valid sea-level terrain when the default no-data value is also zero", async () => {
    // The sample metadata marks this as valid sea level, even though the numeric no-data sentinel is also zero.
    const ground = groundWithQuery(vi.fn(async () => ({
      geometry: { z: 0 }, noDataValue: 0, sampleInfo: [{ demResolution: 1 }],
    })));
    await expect(queryInitialGroundElevation(ground, point)).resolves.toBe(0);
  });
  ...
  it("passes the same AbortSignal to the initial ground query", async () => {
    ...
    // Both loading and querying must observe the caller's cancellation so initialization can stop as one unit.
