/**
 * Runs functional browser checks for selected ArcGIS SDK/loader/projection
 * combinations. The local server exposes only the built component and SDK test
 * fixtures; each headed Chromium run checks initialization, keyboard motion,
 * pause/resume, cockpit roll, SDK reuse, teardown, and captures evidence files.
 * Override SDK_FUNCTIONAL_CASES and SDK_FUNCTIONAL_OUTPUT to select cases/output.
 */
...
// Each case is SDK version : loader mode : spatial reference : optional terrain.
...
// Route requests to the component bundle or isolated SDK fixtures. Reject any
// path that resolves outside its selected root before touching the filesystem.
...
const results = [];
/** Throws immediately when a browser compatibility condition is not satisfied. */
const assert = (condition, message) => { if (!condition) throw new Error(message); };
/** Computes 3D displacement between two ArcGIS world-coordinate points. */
...
try {
  for (const testCase of cases) {
    ...
    try {
      ...
      // The fixture publishes either its initialized test API or a captured
      // setup error; surface the latter before testing interactive behavior.
      ...
      // Apply native keyboard input to prove real simulation movement, then
      // pause and verify that simulation step and position stop advancing.
      ...
      // Camera interpolation advances from rendered frames, so resume the
      // simulation while collecting evidence that cockpit roll is visible.
