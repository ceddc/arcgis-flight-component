 ...
 *
 * @throws If the configured output is outside this repository or has another name.
 */
...
/**
 * Escapes text inserted into generated HTML attributes or navigation labels.
 *
 * @param {string} value Untrusted text to render.
...
/**
 * Produces plain searchable text from a Markdown source string.
 *
 * @param {string} value Markdown source.
 * @returns {string} Normalized text with markup and code fences removed.
 */
...
/**
 * Creates a stable fragment identifier from rendered heading text.
 *
 * @param {string} value Heading HTML/text.
 * @returns {string} Lowercase dash-separated anchor, or "section" if empty.
 */
...
/**
 * Adds unique IDs and direct-link anchors to Markdown-generated headings.
 *
 * @param {string} html Rendered page content.
 * @returns {string} Content with stable, de-duplicated heading IDs.
 */
...
/**
 * Builds a per-page table of contents from the IDs added to rendered headings.
 * Keeping this derived from the final HTML ensures links match their targets.
 * @param {string} html Rendered page content with heading IDs.
 * @returns {string} Accessible in-page navigation for h2/h3 sections.
 */
function sectionNavigation(html) {
  const headings = [...html.matchAll(/<h([23]) id="([^"]+)">([\s\S]*?)<\/h\1>/g)];
  if (headings.length === 0) return "";
  const links = headings.map(([, level, id, content]) => {
    const label = content
      .replace(/<a\b[^>]*>[\s\S]*?<\/a>/g, "")
      .replace(/<[^>]+>/g, "")
      .replace(/&(#x[\da-f]+|#\d+|amp|lt|gt|quot|apos);/gi, (_entity, value) => {
        ...
        return ({ amp: "&", lt: "<", gt: ">", quot: '"', apos: "'" })[value.toLowerCase()] ?? _entity;
      });
    return `<li class="section-link level-${level}"><a href="#${id}">${escapeHtml(label)}</a></li>`;
  }).join("");
...
/**
 * Rewrites links to documentation pages, demos, or repository source files.
 * External URLs, anchors, root-relative URLs, and paths outside the repo remain
 * as authored.
 *
 * @param {string} html Rendered Markdown.
 * @param {{source: string}} page Source metadata for the current page.
 * @returns {string} HTML with resolvable internal links rewritten.
 */
...
/**
 * Maps relative Markdown image paths into the copied docs/images directory.
 * Other sources are left alone so external/absolute media keeps its URL.
 *
 * @param {string} html Rendered Markdown.
 * @param {{source: string}} page Source metadata for the current page.
 * @returns {string} HTML with documentation image references rewritten.
 */
...
function navigation(activeOutput, sections) {
  const groups = Array.from(new Set(pages.map((page) => page.group).filter(Boolean)));
  return groups.map((group) => {
    const links = pages
      .filter((page) => page.group === group)
      .map((page) => {
        const current = page.output === activeOutput ? ' aria-current="page"' : "";
        return `<li><a href="${page.output}"${current}>${escapeHtml(page.title)}</a>${current ? sections : ""}</li>`;
      })
      .join("");
...
/**
 * Wraps rendered article HTML in the shared responsive docs shell.
 *
 * @param {{title: string, output: string, source: string}} page Page metadata.
 * @param {string} content Resolved article HTML.
 * @param {string} sections In-page links for this article.
  ...
  <body class="calcite-mode-light">
    ...
    <div class="shell">
      <aside class="sidebar" id="site-nav">
        ...
        <nav aria-label="Documentation topics">${navigation(page.output, sections)}</nav>
...
// This output is generated from scratch; validate its location before removal.
...
// Render every catalog entry and create a bounded plain-text search document.
const searchIndex = [];
for (const page of pages) {
  ...
  await writeFile(path.join(outputDirectory, page.output), pageTemplate(page, content, sections), "utf8");
  searchIndex.push({
    title: page.title,
    url: page.output,
