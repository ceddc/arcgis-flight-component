/**
 * Builds the component-only ESM and AMD artifacts plus ESM variants for SDK
 * families with APIs unavailable in earlier releases, then records transfer
 * sizes. The ArcGIS SDK remains external so distributions reuse the host SDK.
 */
...
// Each profile uses Vite's mode-specific aliases to omit modules absent from
// that SDK family; the checks below guard against accidentally bundling them.
...
const sizes = {};
// Report both the on-disk and gzip size of each browser-consumable variant.
